 ...
 * Crea lo scheletro dei template con step/GUID (senza constraints/parsers/messaggi)
 * Constraints e parsers verranno aggiunti dopo in-place
 ...
 * @returns Map<nodeId, DialogueTask> - One template per node (solo struttura, senza constraints/parsers)
 */
export async function createTemplatesFromStructures(
  ...
  const createTemplateForNode = async (node: WizardTaskTreeNode): Promise<DialogueTask> => {
    ...
    // Create template (SENZA constraints/parsers - verranno aggiunti dopo)
    ...
    const template: DialogueTask = {
      ...
      // ❌ NO constraints/parsers qui - verranno aggiunti dopo in-place
