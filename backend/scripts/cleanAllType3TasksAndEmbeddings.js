 ...
 *
 * Questo script:
 * 1. Trova tutti i task con type === 3 nel database factory
 * 2. Cancella i task
 * 3. Cancella anche gli embedding corrispondenti (per evitare orphan embeddings)
 *
 * Usage:
 *   node cleanAllType3TasksAndEmbeddings.js --dry-run    # Solo visualizza cosa verrebbe cancellato
 *   node cleanAllType3TasksAndEmbeddings.js --confirm    # Esegue la cancellazione
 */
...
async function cleanAllType3TasksAndEmbeddings() {
  ...
  if (!isDryRun && !isConfirm) {
    ...
    console.log('\nUsage:');
    console.log('  node cleanAllType3TasksAndEmbeddings.js --dry-run    # Solo visualizza');
    console.log('  node cleanAllType3TasksAndEmbeddings.js --confirm    # Esegue cancellazione');
    process.exit(1);
  }
...
cleanAllType3TasksAndEmbeddings();
