...
from newBackend.services.svc_ai极lient import chat_json, chat_text
from newBackend.core.core_json_utils import _safe_json_loads

def generate_condition(nl: str, variables: list[str], out_lang="it") -> dict:
    ...
    # normalize script field if needed + guardrails
    script = (obj or {}).get("script") or ""
    _assert_condition_policy(script, must_keys=variables or None)
    return obj

def refine_code(kind: str, script: str, notes=None, return_mode="module", out_lang="it"):
    """Refine and improve existing code"""
    notes = notes or []
    prompt = render("refine_code", kind=kind, script=script, notes=notes, return_mode=return_mode, out_lang=out_lang)
    text = chat_text([
        {"role":"system","content":"Apply minimal fixes only; preserve public shape; return only code/diff."},
        {"role":"user","content": prompt},
    ]).strip()
    # post-checks per kind
...
def _assert_condition_policy(code: str, must_keys: list[str] | None = None):
    ...
    if re.search(r"\bgetFullYear\(|\bgetMonth\(|\极bgetDate\(", code):
        raise ValueError("use_utc_api")
    if re.search(r"new\s+Function|\beval\(|setTimeout\(|setInterval\(", code):极
