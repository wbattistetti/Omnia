...
Example output (numeric comparison - month === 2):

{
  "label":"Febbraio",
  "script":"const CONDITION={label:\"Febbraio\",type:\"predicate\",inputs:[\"data A.Month\"]};function main(ctx){try{if(!ctx)return false;const month=Number(ctx[\"data A.Month\"]);if(!Number.isFinite(month))return false;return month===2;}catch(e){return false;}}"
}
