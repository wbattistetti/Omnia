...
- **BookFromAgenda ↔ ConvAI (v4.6):** in SEND a **design-time** serve **`projectId`** (costante per scope Redis). Se il binding SEND è **vuoto**, la compile Omnia (TS + VB) genera e **persiste** `Omnia_<cliente>_<nomeProgetto>_<versione>` da metadata progetto + versione release (vedi `generateProjectId`, `patchBookFromAgendaProjectId`, `BookFromAgendaCompileMutator`). **`conversationId`** e **`forceRefresh`** sono **solo runtime** (`x-runtime-mandatory` in OpenAPI): il backend **rifiuta** richieste senza `conversationId` non vuoto (anche via header); **`forceRefresh`** è booleano (dedotto se omesso: `true` con sorgente agenda, `false` su follow-up). Nessun **`tenantId`** nel contratto HTTP (scope Redis = conversation + project). `optionalApiParams` / `designTimeRequiredApiParams` in spec. **Regola ConvAI webhook:** su campi opzionali `""` = assente (`omnia.convai.optional-empty-string/v1`); gateway strip + OpenAPI/report Omnia. Append prompt in `convaiAgentCreatePayload.ts`; `omnia_conversation_id` da `startAgent`. Normalizzazione HTTP in `bookFromAgendaHttpNormalize.js`. Pannello audit: Impostazioni Omnia → **Genera codici**. Dopo cambio spec, **ri-provisionare** l’agente ElevenLabs.
...
| BookFromAgenda — contratto **v4.6** (`projectId` obbligatorio in SEND a design-time; `conversationId` / `forceRefresh` runtime; `designTimeRequiredApiParams` + `optionalApiParams` in `x-omnia.sendBinding`; snapshot Redis) `POST …/bookfromagenda`, spec `…/openapi.json` | `bookFromAgendaService.js`, `bookFromAgendaSlotCache.js`, `bookFromAgendaHttpNormalize.js`; compile SEND `collectBackendCallIncompleteSendMessages.ts`; tool ConvAI `backendToolDerivation.ts`; ApiServer `ElevenLabsEndpoints.vb`; appendice bridge `AIAgentRuntimeBridgeService.js` |
