      ...
      <div className="p-2">
        {query.trim() && isForNodes && allowCreatePicker ? (
          <div className="grid grid-cols-3 gap-2">
            {[{ key: 'AIAgent', label: 'AI Agent', Icon: Bot, color: '#a855f7' }, { key: 'BackendCall', label: 'BackendCall', Icon: Server, color: '#94a3b8' }, { key: 'DataRequest', label: 'Data', Icon: Ear, color: '#3b82f6' }, { key: 'Message', label: 'Message', Icon: Megaphone, color: '#34d399' }, { key: 'Negotiation', label: 'Negotiation', Icon: CheckCircle, color: '#6366f1' }, { key: 'ProblemClassification', label: 'Problem', Icon: GitBranch, color: '#f59e0b' }, { key: 'Summarizer', label: 'Summarizer', Icon: FileText, color: '#06b6d4' }].map(({ key, label, Icon, color }) => (
              <button
                key={key}
              ...
              >
                ...
                <span className="text-slate-700" style={{ fontSize: '8px', lineHeight: 1.2 }}>{label}</span>
              </button>
            ))}
          </div>
        ) : (query.trim() && !isForNodes && allowCreatePicker ? (
