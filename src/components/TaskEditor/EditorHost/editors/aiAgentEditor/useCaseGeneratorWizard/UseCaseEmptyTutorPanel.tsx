...
import {
  ...
  LABEL_EMPTY_USE_CASE_OR_DIVIDER,
  LABEL_EMPTY_USE_CASE_PASTE_SECTION,
...
import {
  ...
  type EmptyTutorGenerateContext,
...
export function UseCaseEmptyTutorPanel({
  ...
  const generateHintLines = buildEmptyTutorGenerateHintLines(generateContext);
      ...
      <div className="flex w-full flex-1 flex-col gap-5 px-4 py-5 sm:gap-6 sm:px-5 sm:py-6 lg:px-6">
        ...
        {bundleBusy ? (
          ...
          <div className="flex flex-col gap-5 sm:gap-6">
            <section className="w-full space-y-3 py-1">
              <p className="text-center text-sm font-semibold text-violet-100/95 sm:text-base">
                {LABEL_EMPTY_USE_CASE_PASTE_SECTION}
              </p>
              <textarea
                ref={rootDraftRef}
            ...
            {canGenerateFromScratch ? (
              <>
                <div
                  className="flex items-center gap-3 text-xs font-medium uppercase tracking-wider text-slate-500"
                  aria-hidden
                >
                  <span className="h-px min-w-[2rem] flex-1 bg-slate-700/80" />
                  <span>{LABEL_EMPTY_USE_CASE_OR_DIVIDER}</span>
                  <span className="h-px min-w-[2rem] flex-1 bg-slate-700/80" />
                </div>
                ...
                <section className="w-full py-1">
                  <p className="mx-auto max-w-lg text-center text-sm leading-relaxed text-pretty text-slate-300 sm:max-w-xl sm:text-base">
                    <button
                      ...
                      className="font-semibold text-violet-200 underline decoration-violet-400/55 underline-offset-[3px] transition-colors hover:text-violet-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/60 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {LABEL_EMPTY_USE_CASE_CLICK_HERE}
                    </button>{' '}
                    e {generateHintLines.actionPhrase}
                    <br />
                    <span className="text-slate-400">{generateHintLines.basesPhrase}</span>
                  </p>
                </section>
              </>
            ) : null}
