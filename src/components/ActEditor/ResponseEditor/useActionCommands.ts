...
export default function useActionCommands(
  ...
  const editAction = React.useCallback((escalationIdx: number, actionIdx: number, newText: string) => {
    setLocalModel(prev => {
      ...
      try { info('RESPONSE_EDITOR', 'editAction', { escalationIdx, actionIdx, newTextLen: newText?.length || 0 }); } catch { }
  ...
  const deleteAction = React.useCallback((escalationIdx: number, actionIdx: number) => {
    setLocalModel(prev => {
      ...
      try { info('RESPONSE_EDITOR', 'deleteAction', { escalationIdx, actionIdx }); } catch { }
      try { onCommit?.(next); } catch { }
      return next;
    });
  }, [setLocalModel, onCommit]);
  ...
  const moveAction = React.useCallback((fromEscIdx: number, fromActIdx: number, toEscIdx: number, toActIdx: number, position: Position) => {
    setLocalModel(prev => {
      ...
      try { info('RESPONSE_EDITOR', 'moveAction', { fromEscIdx, fromActIdx, toEscIdx, toActIdx, position }); } catch { }
      try { onCommit?.(next); } catch { }
      return next;
    });
  }, [setLocalModel, onCommit]);
  ...
  const dropFromViewer = React.useCallback((incoming: any, to: { escalationIdx: number; actionIdx: number }, position: Position) => {
    setLocalModel(prev => {
      ...
      try { info('RESPONSE_EDITOR', 'dropFromViewer', { to, position, insertIdx, actionId: (newAction as any)?.actionId }); } catch { }
      try { onCommit?.(next); } catch { }
      return next;
    });
  }, [setLocalModel, onCommit]);
  ...
  const appendAction = React.useCallback((escalationIdx: number, action: Action) => {
    setLocalModel(prev => {
      ...
      try { info('RESPONSE_EDITOR', 'appendAction', { escalationIdx, actionId: (action as any)?.actionId }); } catch { }
      try { onCommit?.(next); } catch { }
      return next;
    });
  }, [setLocalModel, onCommit]);
