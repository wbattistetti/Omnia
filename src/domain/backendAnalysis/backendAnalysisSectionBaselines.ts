...
/** Testo canonico per editor/revisione dettaglio parametro (allineato alla colonna Descrizione). */
export function paramDetailSectionText(param: BackendParameterAnalysisRecord): string {
  return (
    param.analysisDetailMarkdown.trim() ||
    param.analysisSummary.trim() ||
    param.descriptionShort.trim()
  );
}
